...
(function () {
  ...
  // Dice strategy defaults — `chance` is win chance %, `condition` is 'above' or 'below'.
  // Target is derived: above → target = 100 - chance; below → target = chance.
  // payoutMult ≈ 99 / chance (Stake takes 1% house edge).
  const STRATEGIES = {
    flat: {
      ...
      html += `
        <div class="strat-card" data-key="${key}">
          ...
          <div class="strat-card-axes">
            ...
            <span class="axis-pill axis-c ${axisC}" data-axis="chance" title="Chance/target adjustment">C</span>
          </div>
  ...
  function createPanel() {
    ...
    panel.innerHTML = `
      <style>
        ...
        #moles-panel .axis-legend .lg-c { background: #FF00FF; box-shadow: 0 0 4px rgba(255,0,255,0.4); }
        ...
        #moles-panel .axis-pill.axis-c.on {
  ...
  let axisToggles = { bet: true, chance: true };
